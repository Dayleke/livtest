...
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.33',
  'unifunc' => 'content_675b7f77ba9e09_76881839',
...
function content_675b7f77ba9e09_76881839 (Smarty_Internal_Template $_smarty_tpl) {
...
function renderResult(leafSearch, res) {
    ...
    grid.setHeaders([
        {name: 'Date', indicatorID: 'date', editable: false, callback: function(data, blob) {
            ...
            if(blob[data.recordID].userID == "<?php echo preg_replace("%(?<!\\\\)'%", "\'",htmlspecialchars_decode($_smarty_tpl->tpl_vars['userID']->value, ENT_QUOTES));?>
") {
    ...
    grid.setPostProcessDataFunc(function(data) {
        ...
        for(let i in data) {
            ...
            if(data[i].submitted == '0'
                && data[i].userID == '<?php echo preg_replace("%(?<!\\\\)'%", "\'",htmlspecialchars_decode($_smarty_tpl->tpl_vars['userID']->value, ENT_QUOTES));?>
') {
                data2.push(data[i]);
            }
...
function main() {
    ...
    leafSearch.setJsPath('<?php echo $_smarty_tpl->tpl_vars['app_js_path']->value;?>
');
...
');
    ...
    query.onSuccess(function(res, resStatus, resJqXHR) {
        ...
        if(extendedQueryState == 0) {
            for(let i in res) {
                if(res[i].userID == "<?php echo preg_replace("%(?<!\\\\)'%", "\'",htmlspecialchars_decode($_smarty_tpl->tpl_vars['userID']->value, ENT_QUOTES));?>
") {
        ...
        if(extendedQueryState == 0
            ...
            query.addTerm('userID', '=', "<?php echo preg_replace("%(?<!\\\\)'%", "\'",htmlspecialchars_decode($_smarty_tpl->tpl_vars['userID']->value, ENT_QUOTES));?>
");
            query.execute();
            return false;
        }
